...
class Parser {
	...
	constructor(installDirectory, saveDirectory) {
		...
		this.ships = this.extractInis(
			path.join('DATA', 'SHIPS', 'shiparch.ini'),
			'\r\nnickname = ',
			'\r\nids_name = '
		);
		this.universe = this.extractInis(
			path.join('DATA', 'UNIVERSE', 'universe.ini'),
			'\r\nnickname = ',
			'\r\nstrid_name = '
		);
		this.factions = this.extractInis(
			path.join('DATA', 'initialworld.ini'),
			'\r\nnickname = ',
			'\r\nids_name = '
		);
		const engines = this.extractInis(
			path.join('DATA', 'EQUIPMENT', 'engine_equip.ini'),
			'\r\nnickname = ',
			'\r\nids_name = '
		);
		const equipment = this.extractInis(
			path.join('DATA', 'EQUIPMENT', 'st_equip.ini'),
			'\r\nnickname = ',
			'\r\nids_name = '
		);
		const weapons = this.extractInis(
			path.join('DATA', 'EQUIPMENT', 'weapon_equip.ini'),
			'\r\nnickname = ',
			'\r\nids_name = '
		);
		const misc = this.extractInis(
			path.join('DATA', 'EQUIPMENT', 'misc_equip.ini'),
			'\r\nnickname = ',
			'\r\nids_name = '
		);
	...
	infocardCheck = () => {
		...
		if (!fs.existsSync(infocardPath)) {
			throw new Error(
				`${infocardPath} does not exist. Please run ${process.cwd()}\\FLInfocardIE.exe and export infocards.txt to this location.`
			);
		} else {
	...
	extractInis = (directory, propertySearch, propertySearch2) => {
		...
		const entries = fs
			.readFileSync(path.join(this.installDirectory, directory), 'utf8')
			.split(/\[.*\]/g);
		const map = new Map();
		for (const line of entries) {
			// Grab the string position just after the 1st search string
			const propertyPosition =
				line.indexOf(propertySearch) + propertySearch.length;
			...
			if (propertyPosition >= propertySearch.length) {
				...
				const propertyPosition2 =
					line.indexOf(propertySearch2) + propertySearch2.length;
				...
				if (property && propertyPosition2 >= propertySearch2.length) {
					...
					const property2 = this.infocards.get(
						line.substring(
							propertyPosition2,
							line.indexOf('\r\n', propertyPosition2)
						)
	...
	sort = (sort, direction) => {
		...
		if (direction == 'Desc') this.players = this.players.reverse();
		return this;
	};
	...
	filter = (range, rangeType) => {
		...
		for (let i = (this.players.length = 1); i >= 0; i--) {
			...
			if (
				(rangeType == 'LastSeen' && this.players[i].lastseen < dateAfter) ||
				(rangeType == 'Created' && this.players[i].created < dateAfter)
			) {
	...
	parsePlayerFiles = () => {
		...
		for (const file of playerFiles) {
			let config = ini.parse(fs.readFileSync(file, 'utf8'), {
				inlineArrays: true,
			});
			...
			if (Object.keys(config).length != 0) {
				...
				if (config.Player) {
					...
					player.system = config.Player.system
						? this.universe.get(config.Player.system.toLowerCase())
						: null;
					...
					player.internalShip = config.Player.ship_archetype
						? this.hash.getNickname(Number(config.Player.ship_archetype))
						: null;
					player.ship = player.internalShip
						? this.ships.get(player.internalShip.toLowerCase())
						: null;
					player.internalBase = config.Player.base ? config.Player.base : null;
					player.base = config.Player.base
						? this.universe.get(config.Player.base.toLowerCase())
						: 'In Space';
					player.internalFaction = config.Player.rep_group
						? config.Player.rep_group
						: null;
					player.faction = config.Player.rep_group
						? this.factions.get(config.Player.rep_group)
						: 'Freelancer';

					if (config.mPlayer) {
						player.timePlayed = config.mPlayer.total_time_played
							? config.mPlayer.total_time_played
							: 0;
						player.basesVisited = config.mPlayer.base_visited
							? Array.isArray(config.mPlayer.base_visited)
								? config.mPlayer.base_visited.length
								: 1
							: 0;
						player.systemsVisited = config.mPlayer.sys_visited
							? Array.isArray(config.mPlayer.sys_visited)
								? config.mPlayer.sys_visited.length
								: 1
							: 0;
						player.holesVisited = config.mPlayer.holes_visited
							? Array.isArray(config.mPlayer.holes_visited)
								? config.mPlayer.holes_visited.length
								: 1
							: 0;
						...
						if (config.mPlayer.rm_completed) {
							if (Array.isArray(config.mPlayer.rm_completed)) {
								...
								player.missions = parseInt(
									config.mPlayer.rm_completed.split(',')[1]
								);
						}
						...
						if (config.mPlayer.ship_type_killed) {
							if (Array.isArray(config.mPlayer.ship_type_killed)) {
								...
								player.kills = parseInt(
									config.mPlayer.ship_type_killed.split(',')[1]
								);
						}
					}
					...
					if (config.Player.equip) {
						...
						for (const equipString of config.Player.equip) {
							...
							if (hardpoint.toLowerCase().includes('light')) {
								...
								const nickname =
									this.equipment.get(internalNickname) || internalNickname;
								if (
									internalNickname &&
									!internalNickname.includes('contrail')
								) {
